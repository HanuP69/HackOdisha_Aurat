...
//   useEffect(() => {
//     if ((message && message.lipsync) || (audioUrl && visemes?.mouthCues?.length)) {
...
//         } else if (time > lipsync.mouthCues[lipsync.mouthCues.length - 1].end) {
...
//   return (
//     <group ref={group} rotation={[0.3, 0, 0]} scale={[2, 2, 2]} position={[0, -2, 0]}>
...
function Model({ audioUrl, visemes, emotion, message }) {
  ...
  // Adjust model scale, position, and rotation
  return (
    <group ref={group} rotation={[0.0, 0, 0]} scale={[2, 2, 2]} position={[0, -2, 0]}>
      <primitive object={nodes.Scene} />
    </group>
  );
}
